...
public class AddBusBar extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_add_bus_bar);

//        BusBarConType =(Spinner)findViewById(R.id.busBarConductorType);
...
//        BusBarConType.setAdapter(adapter1);
        ...
        if (!isConnected(AddBusBar.this)) {
            //readExcel();
            //readExcelLoadtype();
            //readExcelConType();
        } else {
            new AddBusBar.loadProvince().executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
            new AddBusBar.loadConType().executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
        }
        ...
        SpinnerProvince.setOnItemSelectedListener(new Spinner.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                if(!Util.isConnected(AddBusBar.this)){
                    //readExcelLine();
                    Toast.makeText(getApplication(),"readExcel1: " , Toast.LENGTH_SHORT).show();

                }else {
                    new AddBusBar.loadAreaByProvince().executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
                }
            }
        ...
        SpinnerArea.setOnItemSelectedListener(new Spinner.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                if(!Util.isConnected(AddBusBar.this)){
                    ...
                    //new AddBusBar.loadLineByArea().executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
                    new AddBusBar.loadGantrybyArea().executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
                }
            }
            public void onNothingSelected(AdapterView<?> parent) {
            }
...
//        });
            ...
            public void onClick(View view) {

                {
                    ...
                    if (NoOfTransformers.getText().toString().trim().equals("")) {
                        ...
                        AlertDialog.Builder builder = new AlertDialog.Builder(AddBusBar.this);
                                ...
                                new DialogInterface.OnClickListener() {
                                    ...
                                    public void onClick(DialogInterface dialog, int which) {
                                        if(!isConnected(AddBusBar.this)){
                                            ...
                                            //new AddBusBar.loadGantryObj().executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
                                            new AddBusBar.AddBusBarDB().executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
    ...
    private class AddBusBarDB extends AsyncTask<Void, Void, String>{
        ...
        protected String doInBackground(Void... voids) {
            ...
            objAddgantry.setBusBarCondutoer(new BigDecimal(contype));
            objAddgantry.setBusBarInsulator(new BigDecimal(busBarIns));

            if(noTransformers.length()==0){
                objAddgantry.setBusBarNoTranformer(new BigDecimal(0));
            }else{
                objAddgantry.setBusBarNoTranformer(new BigDecimal(noTransformers));
            }

            if(noLbs.length()==0){
                objAddgantry.setBusBarLbs(new BigDecimal(0));
            }else{
                objAddgantry.setBusBarLbs(new BigDecimal(noLbs));
            }

            if(noAbs.length()==0){
                objAddgantry.setBusBarAbs(new BigDecimal(0));
            }else{
                objAddgantry.setBusBarAbs(new BigDecimal(noAbs));
            }

            if(noDdloSet.length()==0){
                objAddgantry.setBusBarDdlo(new BigDecimal(0));
            }else{
                objAddgantry.setBusBarDdlo(new BigDecimal(noDdloSet));
            }

            if(noSurge.length()==0){
                objAddgantry.setBusBarSa(new BigDecimal(0));
            }else {
                objAddgantry.setBusBarSa(new BigDecimal(noSurge));
            }

            System.out.println("Set object testing :****************************************");
            System.out.println("Bus Bar Con type: "+objAddgantry.getBusBarCondutoer());
            System.out.println("Bus Bar Ins: "+objAddgantry.getBusBarInsulator());
            System.out.println("No Transformers:" + objAddgantry.getBusBarNoTranformer());
            System.out.println("No LBS: "+objAddgantry.getBusBarLbs());
            System.out.println("No ABS: "+objAddgantry.getBusBarAbs());
            System.out.println("No DDLO Sets: "+objAddgantry.getBusBarDdlo());
            System.out.println("No Surge: "+objAddgantry.getBusBarSa());
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        ...
        if (id == R.id.apphome) {
            Intent intent = new Intent(AddBusBar.this, MainActivity.class);
            startActivity(intent);

        } else if (id == R.id.nearby) {
            Intent intent = new Intent(AddBusBar.this, GetNearByTower.class);
            startActivity(intent);

            //////////////////////////////// PHM - LCM ////////////////////////////////////////////

        } else if (id == R.id.nav_addLine) {
            Intent intent = new Intent(AddBusBar.this, AddLine.class);
            startActivity(intent);

        } else if (id == R.id.nav_addSupport) {
            Intent intent = new Intent(AddBusBar.this, AddSupport.class);
            startActivity(intent);

        }else if (id == R.id.nav_addTowerMaintainance) {
            Intent intent = new Intent(AddBusBar.this, TM2.class);
            startActivity(intent);

            //////////////////////////////// PHM - SUb ////////////////////////////////////////////

        } else if (id == R.id.nav_addGantry) {
            Intent intent = new Intent(AddBusBar.this, AddGantry.class);
            startActivity(intent);

        } else if (id == R.id.nav_addBusBar) {
            Intent intent = new Intent(AddBusBar.this, AddBusBar.class);
            startActivity(intent);
        } else if (id == R.id.nav_editBusBar) {
            Intent intent = new Intent(AddBusBar.this, EditBusBar.class);
            startActivity(intent);

        } else if (id == R.id.nav_addStructual) {
            Intent intent = new Intent(AddBusBar.this, AddStructural.class);
            startActivity(intent);

        } else if (id == R.id.nav_addGantryMore) {
            Intent intent = new Intent(AddBusBar.this, AddGantryMore.class);
            startActivity(intent);

        } else if (id == R.id.nav_addFeeder) {
            Intent intent = new Intent(AddBusBar.this, AddFeeder.class);
            startActivity(intent);

        } else if (id == R.id.nav_addSwitches) {
            Intent intent = new Intent(AddBusBar.this, AddSwitches.class);
            startActivity(intent);

        } else if (id == R.id.nav_addSurge) {
            Intent intent = new Intent(AddBusBar.this, AddSurgeArrestors.class);
            startActivity(intent);

        } else if (id == R.id.nav_addTransformersG) {
            Intent intent = new Intent(AddBusBar.this, AddTransformersG.class);
            startActivity(intent);

        }else if (id == R.id.nav_addEquipment) {
            Intent intent = new Intent(AddBusBar.this, AddEquipment.class);
            startActivity(intent);

///////////////////// POLE DETAILS //////////////////////////////////////////////

        } else if (id == R.id.nav_addMVPoles) {
            Intent intent = new Intent(AddBusBar.this, AddMVPoles.class);
            startActivity(intent);

        } else if (id == R.id.nav_addPoles) {
            Intent intent = new Intent(AddBusBar.this, AddPoles.class);
            startActivity(intent);

        } else if (id == R.id.nav_addTowers) {
            Intent intent = new Intent(AddBusBar.this, AddTransformers.class);
            startActivity(intent);

        } else if (id == R.id.nav_Login) {
            Intent intent = new Intent(AddBusBar.this, Login.class);
            startActivity(intent);
        }
    ...
    private class loadProvince extends AsyncTask<String, Void, Glcompm[]>{
        ...
        protected void onPreExecute() {
            ...
            ProgDialog= new ProgressDialog(AddBusBar.this);
