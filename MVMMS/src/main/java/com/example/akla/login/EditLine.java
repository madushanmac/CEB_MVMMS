...
public class EditLine extends AppCompatActivity
    ...
    //load line Code
    Spinner SpinnerLineCode;
    HashMap<Integer, String> spinnerMapLineCode = new HashMap<Integer, String>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btn_date.setOnClickListener(EditLine.this);


        if (!isConnected(EditLine.this)) {
            readExcel();
            readExcelLoadtype();
            readExcelConType();
        } else {
            new EditLine.loadarea().executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
            new EditLine.loadtype().executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
            new EditLine.loadConType().executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
...
//        setDateTimeField();
        ...
        SpinnerArea.setOnItemSelectedListener(new Spinner.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                new EditLine.loadLineCode().executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
        ...
        SpinnerLineCode = findViewById(R.id.SpinnerLineCode);
        SpinnerLineCode.setOnItemSelectedListener(new Spinner.OnItemSelectedListener() {

            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                String selectedItem = parent.getSelectedItem().toString();
                String name = parent.getSelectedItem().toString();
                String  idline = spinnerMapLineCode.get(parent.getSelectedItemPosition());
                lineCode = idline;
                System.out.println(lineCode+"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");

                if(!Util.isConnected(EditLine.this)){
                    //readExcelLine();
                    Toast.makeText(getApplication(),"readExcel1: " , Toast.LENGTH_SHORT).show();

                }else {
                    new EditLine.loadLineObj().executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
                }
            }
            public void onNothingSelected(AdapterView<?> parent) {
            }
            ...
            public void onClick(View v) {
                {
                    ...
                    if (ContextCompat.checkSelfPermission(EditLine.this.getApplicationContext(),
                            Manifest.permission.WRITE_EXTERNAL_STORAGE)
                            != PackageManager.PERMISSION_GRANTED) {

                        // If you do not have permission, request it
                        ActivityCompat.requestPermissions(EditLine.this,
...
//                        Toast.LENGTH_LONG).show();
                    ...
                    if (Name.getText().toString().trim().equals("")) {
                        ...
                        AlertDialog.Builder builder = new AlertDialog.Builder(EditLine.this);
            ...
            public void onClick(View view) {

                {
                    if (objAddline != null) {
                        ...
                        if (objAddline.getLineName().toString() == null) {
                            Name.setText("-");
                        } else {
                            Name.setText(objAddline.getLineName().toString());
                        }

                        if (objAddline.getLength().toString() == null) {
                            Length.setText("-");
                        } else {
                            Length.setText(objAddline.getLength().toString());
                        }

                        if (objAddline.getNoofpoles().toString() == null) {
                            NoOfPoles.setText("-");
                        } else {
                            NoOfPoles.setText(objAddline.getNoofpoles().toString());
                        }

                        if (objAddline.getNooftowers().toString() == null) {
                            NoOfTowers.setText("-");
                        } else {
                            NoOfTowers.setText(objAddline.getNooftowers().toString());
                        }

                        if (objAddline.getComdate().toString() == null) {
                            ComDate.setText("-");
                        } else {
                            ComDate.setText((objAddline.getComdate().getYear() - 100 + 2000)
                                    + "-" + (objAddline.getComdate().getMonth() + 1)
                                    + "-" + (objAddline.getComdate().getDate()));
                        }
    ...
    public void alertMessage() {
        Toast.makeText(EditLine.this, "Error",
                Toast.LENGTH_LONG).show();
...
//            startActivity(intent);
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        ...
        if (id == R.id.apphome) {
            Intent intent = new Intent(EditLine.this, MainActivity.class);
            startActivity(intent);

        } else if (id == R.id.nearby) {
            Intent intent = new Intent(EditLine.this, GetNearByTower.class);
            startActivity(intent);

            //////////////////////////////// PHM - LCM ////////////////////////////////////////////

        } else if (id == R.id.nav_addLine) {
            Intent intent = new Intent(EditLine.this, EditLine.class);
            startActivity(intent);

        } else if (id == R.id.nav_addSupport) {
            Intent intent = new Intent(EditLine.this, AddSupport.class);
            startActivity(intent);

        }else if (id == R.id.nav_addTowerMaintainance) {
            Intent intent = new Intent(EditLine.this, TM2.class);
            startActivity(intent);

            //////////////////////////////// PHM - SUb ////////////////////////////////////////////

        } else if (id == R.id.nav_addGantry) {
            Intent intent = new Intent(EditLine.this, AddGantry.class);
            startActivity(intent);

        } else if (id == R.id.nav_addBusBar) {
            Intent intent = new Intent(EditLine.this, AddBusBar.class);
            startActivity(intent);

        } else if (id == R.id.nav_addStructual) {
            Intent intent = new Intent(EditLine.this, AddStructural.class);
            startActivity(intent);

        } else if (id == R.id.nav_addGantryMore) {
            Intent intent = new Intent(EditLine.this, AddGantryMore.class);
            startActivity(intent);

        } else if (id == R.id.nav_addFeeder) {
            Intent intent = new Intent(EditLine.this, AddFeeder.class);
            startActivity(intent);

        } else if (id == R.id.nav_addSwitches) {
            Intent intent = new Intent(EditLine.this, AddSwitches.class);
            startActivity(intent);

        } else if (id == R.id.nav_addSurge) {
            Intent intent = new Intent(EditLine.this, AddSurgeArrestors.class);
            startActivity(intent);

        } else if (id == R.id.nav_addTransformersG) {
            Intent intent = new Intent(EditLine.this, AddTransformersG.class);
            startActivity(intent);

        }else if (id == R.id.nav_addEquipment) {
            Intent intent = new Intent(EditLine.this, AddEquipment.class);
            startActivity(intent);

///////////////////// POLE DETAILS //////////////////////////////////////////////

        } else if (id == R.id.nav_addMVPoles) {
            Intent intent = new Intent(EditLine.this, AddMVPoles.class);
            startActivity(intent);

        } else if (id == R.id.nav_addPoles) {
            Intent intent = new Intent(EditLine.this, AddPoles.class);
            startActivity(intent);

        } else if (id == R.id.nav_addTowers) {
            Intent intent = new Intent(EditLine.this, AddTransformers.class);
            startActivity(intent);

        } else if (id == R.id.nav_Login) {
            Intent intent = new Intent(EditLine.this, Login.class);
            startActivity(intent);
        }
...
//                Toast.LENGTH_LONG).show();
        ...
        android.app.AlertDialog.Builder builder1 = new android.app.AlertDialog.Builder(EditLine.this);
...
//            System.out.println(" areaCode " + areaCode);
            ...
            final String url1 = Util.SRT_URL + "MMSUpdateLineMobile";
...
//
    ...
    public void onClick(View v) {
        ...
        if (v == btn_date) {
            ...
            DatePickerDialog datePickerDialog = new DatePickerDialog(EditLine.this,
    ...
    ////////// L O A D  Line Code /////////////////////////// R O W D Y //////////////////////////
    private class loadLineCode extends AsyncTask<String, Void, MmsAddline[]>{
        @Override
        protected MmsAddline[] doInBackground(String... strings) {

            RestTemplate rest = new RestTemplate();
            //String url6 = Util.SRT_URL+"findAllAreaNew";
            String url6 = Util.SRT_URL+"findLineByAreaStatusNew/"+area+"/2/";

            System.out.println(area+" QQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQ");
            rest.getMessageConverters().add(new MappingJackson2HttpMessageConverter());
            return rest.getForObject(url6,MmsAddline[].class);
        }
        ...
        protected void onPostExecute(MmsAddline[]  results) {
            ...
            System.out.println("pPPPPPPPPPPPPPPPpppppppppppppppppp: "+results.length+" :PPPPPPPPPPPPPPPPPPppppppppppppPPPPPPPPPP"+results);
            String[] province;
            ...
            if(results != null){
                ...
                for(int c =0; c <=count; c++){
                    ...
                    spinnerMapLineCode.put(c,obj.getCode());
                    System.out.println("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"+obj.getLineName()+" : kkk: "+ obj.getCode());
                }
            }
            ArrayAdapter<String> adapterPr = new ArrayAdapter< String>(getBaseContext(), android.R.layout.simple_spinner_dropdown_item, valuesLine);
            adapterPr.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
            SpinnerLineCode = findViewById(R.id.SpinnerLineCode);
            SpinnerLineCode.setAdapter(adapterPr);
        }
    }


    ////////// L O A D   L I N E   O B J E C T /////////////////////////////////////////////////////////
    private class loadLineObj extends AsyncTask<String, Void, MmsAddline> {

        @Override
        protected MmsAddline doInBackground(String... urls) {

            RestTemplate rest = new RestTemplate();
            String url6 = Util.SRT_URL + "findLineByIdNew/" +lineCode+"/";
            System.out.println(lineCode+" :QQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQ");

            rest.getMessageConverters().add(new MappingJackson2HttpMessageConverter());
            System.out.println("VVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVV");

            objAddline =  rest.getForObject(url6, MmsAddline.class);;
            System.out.println("KKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKK"+objAddline.getLineName());
            return rest.getForObject(url6, MmsAddline.class);
        }
    }
